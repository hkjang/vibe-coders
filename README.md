...
- **런타임 관리자 설정 — 기반** (`v0.8.0`): ClickHouse·Text2SQL 설정을 `env 기본값 + DB 관리자 오버레이`로 관리하는 기반(`/admin/settings` 조회·`/by-key/{key}` PUT/DELETE·`/validate`·`/rollback`·`/history`) — 민감값 암호화 저장 + `********` 마스킹, 변경 이력·롤백·교차 검증. 런타임 즉시 반영은 후속 릴리스
