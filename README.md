...
```bash
docker build --build-arg VERSION=v0.84.1 -t ai-coding-proxy-gateway:v0.84.1 .
UI_APP_ENABLED=true GATEWAY_VERSION=v0.84.1 ./scripts/init-deployment-env.sh .env
...
```powershell
pwsh -File scripts/release.ps1 -Version v0.84.1
```

```bash
./scripts/release.sh -v v0.84.1 -p linux/amd64
```
...
```
release/
  ai-coding-proxy-gateway-v0.84.1.tar.gz
  ai-coding-proxy-gateway-v0.84.1.tar.gz.sha256
  README-offline-v0.84.1.md
  SBOM-v0.84.1.spdx.json
  THIRD_PARTY_LICENSES-v0.84.1.md
  init-deployment-env-v0.84.1.sh
  backup-volume-v0.84.1.sh
```
...
2. 체크섬 확인
   ...
   ```bash
   sha256sum -c ai-coding-proxy-gateway-v0.84.1.tar.gz.sha256
   ```
...
4. 최초 1회 비밀값 파일과 데이터 볼륨을 만든 뒤 실행
   ...
   ```bash
   chmod 0700 init-deployment-env-v0.84.1.sh backup-volume-v0.84.1.sh
   sudo env GATEWAY_VERSION=v0.84.1 \
     ./init-deployment-env-v0.84.1.sh /opt/proxy-gateway/gateway.env
   ...
   docker run --rm --user 0:0 --mount source=proxy-gateway-data,target=/data \
       ai-coding-proxy-gateway:v0.84.1 repair-data-dir
...
5. 또는 저장소에서 별도로 검토·전달한 `docker-compose.yml` 과 함께 운영
   ...
   ```bash
   GATEWAY_VERSION=v0.84.1 ./init-deployment-env-v0.84.1.sh .env
   docker compose up -d
   ```
...
```bash
docker run --rm --mount source=proxy-gateway-data,target=/data ai-coding-proxy-gateway:v0.84.1 check-data-dir
docker run --rm --user 0:0 --mount source=proxy-gateway-data,target=/data ai-coding-proxy-gateway:v0.84.1 repair-data-dir
...
```bash
bash scripts/container-smoke.sh ai-coding-proxy-gateway:v0.84.1 v0.84.1
```
