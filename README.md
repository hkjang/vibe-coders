<p align="center">
  <img src="docs/images/logo.svg" alt="vibe-coders logo" width="150" />
</p>
...
```bash
docker build --build-arg VERSION=v0.85.3 -t ai-coding-proxy-gateway:v0.85.3 .
UI_APP_ENABLED=true GATEWAY_VERSION=v0.85.3 ./scripts/init-deployment-env.sh .env
...
5. 또는 저장소에서 별도로 검토·전달한 `docker-compose.yml` 과 함께 운영
   ...
   ```bash
   GATEWAY_VERSION=v0.85.3 ./init-deployment-env-v0.85.3.sh .env
   docker compose up -d
   ```

데이터는 `/data` 볼륨에 SQLite 파일과 fallback NDJSON 로 보관되므로 컨테이너를 재기동해도 누적 통계가 유지됩니다.

컨테이너가 재시작을 반복하며 8080이 열리지 않고 `docker logs`에 `data directory /data is not writable`
또는 `attempt to write a readonly database`가 보이면, `/data`를 root 등 다른 사용자가 기록한 상태입니다
(바인드 마운트, 이전 배포의 볼륨, Kubernetes PVC). 게이트웨이는 nonroot(uid 65532)로 실행되고 이미지에는
셸이 없으므로 같은 이미지로 한 번만 소유권을 복구합니다. 새 볼륨에서는 아무것도 바꾸지 않으므로 항상 실행해도 됩니다.

```bash
docker run --rm --mount source=proxy-gateway-data,target=/data ai-coding-proxy-gateway:v0.85.3 check-data-dir
docker run --rm --user 0:0 --mount source=proxy-gateway-data,target=/data ai-coding-proxy-gateway:v0.85.3 repair-data-dir
docker restart proxy-gateway
```

자세한 절차는 `docs/OPERATIONS.md` 8.6절을 참고하십시오.
