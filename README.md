...
```bash
docker build --build-arg VERSION=v0.85.4 -t ai-coding-proxy-gateway:v0.85.4 .
UI_APP_ENABLED=true GATEWAY_VERSION=v0.85.4 ./scripts/init-deployment-env.sh .env
...
```powershell
pwsh -File scripts/release.ps1 -Version v0.85.4
```

```bash
./scripts/release.sh -v v0.85.4 -p linux/amd64
```
...
```
release/
  ai-coding-proxy-gateway-v0.85.4.tar.gz
  ai-coding-proxy-gateway-v0.85.4.tar.gz.sha256
  README-offline-v0.85.4.md
  SBOM-v0.85.4.spdx.json
  THIRD_PARTY_LICENSES-v0.85.4.md
  init-deployment-env-v0.85.4.sh
  backup-volume-v0.85.4.sh
```
...
2. 체크섬 확인
   ...
   ```bash
   sha256sum -c ai-coding-proxy-gateway-v0.85.4.tar.gz.sha256
   ```
...
4. 최초 1회 비밀값 파일과 데이터 볼륨을 만든 뒤 실행
   ...
   ```bash
   chmod 0700 init-deployment-env-v0.85.4.sh backup-volume-v0.85.4.sh
   sudo env GATEWAY_VERSION=v0.85.4 \
     ./init-deployment-env-v0.85.4.sh /opt/proxy-gateway/gateway.env
   ...
   docker run --rm --user 0:0 --mount source=proxy-gateway-data,target=/data \
       ai-coding-proxy-gateway:v0.85.4 repair-data-dir
...
5. 또는 저장소에서 별도로 검토·전달한 `docker-compose.yml` 과 함께 운영
   ...
   ```bash
   GATEWAY_VERSION=v0.85.4 ./init-deployment-env-v0.85.4.sh .env
   docker compose up -d
   ```
...
```bash
docker run --rm --mount source=proxy-gateway-data,target=/data ai-coding-proxy-gateway:v0.85.4 check-data-dir
docker run --rm --user 0:0 --mount source=proxy-gateway-data,target=/data ai-coding-proxy-gateway:v0.85.4 repair-data-dir
...
```bash
bash scripts/container-smoke.sh ai-coding-proxy-gateway:v0.85.4 v0.85.4
```
