...
| 파일 | 검사 | 의존성 |
|---|---|---|
| `admin_ui_static_test.go` | raw string 무결성(내부 백틱), `alert()` 잔존, 읽기 전용 엔드포인트의 성공 알림, 토스트의 `innerHTML` 사용, 오버레이 dialog 시맨틱·포커스 헬퍼, **빈/로딩/오류 스타일 분리**, **원시 타임스탬프 노출** | 없음 (항상 실행) |
| `admin_ui_behavior_test.go` + `testdata/admin_ui_behavior.js` | 토스트 동작, **시간 포맷팅**, 명령 팔레트 매칭·인덱스, 포커스 가둠·복원, 표 래핑 멱등성 — 스텁 DOM에서 실제 함수를 실행 | node (없으면 skip) |
