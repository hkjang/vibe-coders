...
릴리즈 자산(GitHub Release `v0.85.4`)으로 처음부터 끝까지. 빌드 호스트에서 이미지를 만드는 절차와 오프라인망 적재 배경은 [OPERATIONS.md 2.5](OPERATIONS.md#25-오프라인망-적재)에 있습니다.
...
sudo bash init-deployment-env-v0.85.4.sh /opt/proxy-gateway/gateway.env
...
- **백업·복구**: `/data` 볼륨이 전부입니다. `backup-volume-v0.85.4.sh` 로 tar 백업, 복구는 볼륨을 유지한 채 컨테이너만 교체합니다 — [OPERATIONS.md 6](OPERATIONS.md#6-백업--복구).
