...
VITE_UI_VERSION=v0.85.3 pnpm --dir web build
...
go run -ldflags "-X vibe-coders/internal/proxy.AppVersion=v0.85.3" ./cmd/gateway
```
...
```powershell
pwsh -File scripts/release.ps1 -Version v0.85.3
```

### Linux / macOS

```bash
./scripts/release.sh -v v0.85.3 -p linux/amd64
```
...
```
release/
  ai-coding-proxy-gateway-v0.85.3.tar.gz        ← Docker 이미지 패키지
  ai-coding-proxy-gateway-v0.85.3.tar.gz.sha256 ← SHA256 체크섬
  README-offline-v0.85.3.md                      ← 오프라인 배포 가이드
  SBOM-v0.85.3.spdx.json                         ← Go+npm 통합 SPDX SBOM
  THIRD_PARTY_LICENSES-v0.85.3.md                ← Go·Frontend 라이선스 목록
  init-deployment-env-v0.85.3.sh                 ← 운영 env 원자 생성·검증
  backup-volume-v0.85.3.sh                       ← named volume·env 백업·복구
```
...
# 이미지 이름 변경
pwsh -File scripts/release.ps1 -Version v0.85.3 -Image my-gateway

# ARM64 빌드 (애플 실리콘 / ARM 서버)
pwsh -File scripts/release.ps1 -Version v0.85.3 -Platform linux/arm64
```
...
```powershell
# 스크립트를 사용하여 릴리즈 업로드
pwsh -File scripts/gh_release.ps1 -Version v0.85.3 -PrevVersion v0.82.2
```
...
```powershell
gh release view v0.85.3 --repo hkjang/vibe-coders
```
...
```
ai-coding-proxy-gateway-v0.85.3.tar.gz
ai-coding-proxy-gateway-v0.85.3.tar.gz.sha256
README-offline-v0.85.3.md
SBOM-v0.85.3.spdx.json
THIRD_PARTY_LICENSES-v0.85.3.md
init-deployment-env-v0.85.3.sh
backup-volume-v0.85.3.sh
```

### 7.2 무결성 확인

```bash
sha256sum -c ai-coding-proxy-gateway-v0.85.3.tar.gz.sha256
# 정상: ai-coding-proxy-gateway-v0.85.3.tar.gz: OK
```

### 7.3 이미지 적재

```bash
gunzip -c ai-coding-proxy-gateway-v0.85.3.tar.gz | docker load
# 정상: Loaded image: ai-coding-proxy-gateway:v0.85.3
```

### 7.4 단일 컨테이너 실행

```bash
chmod 0700 init-deployment-env-v0.85.3.sh backup-volume-v0.85.3.sh
sudo env GATEWAY_VERSION=v0.85.3 \
  ./init-deployment-env-v0.85.3.sh /opt/proxy-gateway/gateway.env
...
docker run --rm --user 0:0 --mount source=proxy-gateway-data,target=/data \
  ai-coding-proxy-gateway:v0.85.3 repair-data-dir
docker run -d --name proxy-gateway --restart=always \
  ...
  ai-coding-proxy-gateway:v0.85.3
```
...
```bash
bash scripts/container-smoke.sh ai-coding-proxy-gateway:v0.85.3 v0.85.3
```
...
scripts/backup-volume.sh restore \
  --image ai-coding-proxy-gateway:v0.85.3 \
