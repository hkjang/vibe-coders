...
go run -ldflags "-X vibe-coders/internal/proxy.AppVersion=v0.85.0" ./cmd/gateway
```
...
```
release/
  ai-coding-proxy-gateway-v0.85.0.tar.gz        ← Docker 이미지 패키지
  ...
  THIRD_PARTY_LICENSES-v0.85.0.md                ← Go·Frontend 라이선스 목록
  init-deployment-env-v0.85.0.sh                 ← 운영 env 원자 생성·검증
  backup-volume-v0.85.0.sh                       ← named volume·env 백업·복구
```
...
# 이미지 이름 변경
pwsh -File scripts/release.ps1 -Version v0.85.0 -Image my-gateway

# ARM64 빌드 (애플 실리콘 / ARM 서버)
pwsh -File scripts/release.ps1 -Version v0.85.0 -Platform linux/arm64
```
...
```powershell
# 스크립트를 사용하여 릴리즈 업로드
pwsh -File scripts/gh_release.ps1 -Version v0.85.0 -PrevVersion v0.82.2
```
...
```powershell
gh release view v0.85.0 --repo hkjang/vibe-coders
```
...
# 정상: ai-coding-proxy-gateway-v0.85.0.tar.gz: OK
```
...
# 정상: Loaded image: ai-coding-proxy-gateway:v0.85.0
```
