...
go run -ldflags "-X vibe-coders/internal/proxy.AppVersion=v0.85.1" ./cmd/gateway
```
...
```powershell
pwsh -File scripts/release.ps1 -Version v0.85.1
```
...
```
release/
  ai-coding-proxy-gateway-v0.85.1.tar.gz        ← Docker 이미지 패키지
  ai-coding-proxy-gateway-v0.85.1.tar.gz.sha256 ← SHA256 체크섬
  README-offline-v0.85.1.md                      ← 오프라인 배포 가이드
  SBOM-v0.85.1.spdx.json                         ← Go+npm 통합 SPDX SBOM
  THIRD_PARTY_LICENSES-v0.85.1.md                ← Go·Frontend 라이선스 목록
  init-deployment-env-v0.85.1.sh                 ← 운영 env 원자 생성·검증
  backup-volume-v0.85.1.sh                       ← named volume·env 백업·복구
```
...
# 이미지 이름 변경
pwsh -File scripts/release.ps1 -Version v0.85.1 -Image my-gateway

# ARM64 빌드 (애플 실리콘 / ARM 서버)
pwsh -File scripts/release.ps1 -Version v0.85.1 -Platform linux/arm64
```
...
```powershell
# 스크립트를 사용하여 릴리즈 업로드
pwsh -File scripts/gh_release.ps1 -Version v0.85.1 -PrevVersion v0.82.2
```
...
```powershell
gh release view v0.85.1 --repo hkjang/vibe-coders
```
...
```
ai-coding-proxy-gateway-v0.85.1.tar.gz
ai-coding-proxy-gateway-v0.85.1.tar.gz.sha256
README-offline-v0.85.1.md
SBOM-v0.85.1.spdx.json
THIRD_PARTY_LICENSES-v0.85.1.md
init-deployment-env-v0.85.1.sh
backup-volume-v0.85.1.sh
```
...
# 정상: ai-coding-proxy-gateway-v0.85.1.tar.gz: OK
```
...
# 정상: Loaded image: ai-coding-proxy-gateway:v0.85.1
```

### 7.4 단일 컨테이너 실행

```bash
chmod 0700 init-deployment-env-v0.85.1.sh backup-volume-v0.85.1.sh
sudo env GATEWAY_VERSION=v0.85.1 \
  ./init-deployment-env-v0.85.1.sh /opt/proxy-gateway/gateway.env
...
```bash
bash scripts/container-smoke.sh ai-coding-proxy-gateway:v0.85.1 v0.85.1
```
