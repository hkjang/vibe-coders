...
go run -ldflags "-X vibe-coders/internal/proxy.AppVersion=v0.80.0" ./cmd/gateway
```
...
```
release/
  ai-coding-proxy-gateway-v0.80.0.tar.gz        ← Docker 이미지 패키지
  ai-coding-proxy-gateway-v0.80.0.tar.gz.sha256 ← SHA256 체크섬
  README-offline-v0.80.0.md                      ← 오프라인 배포 가이드
  SBOM-v0.80.0.spdx.json                         ← Go+npm 통합 SPDX SBOM
  THIRD_PARTY_LICENSES-v0.80.0.md                ← Go·Frontend 라이선스 목록
  init-deployment-env-v0.80.0.sh                 ← 운영 env 원자 생성·검증
  backup-volume-v0.80.0.sh                       ← named volume·env 백업·복구
```
...
# 이미지 이름 변경
pwsh -File scripts/release.ps1 -Version v0.80.0 -Image my-gateway

# ARM64 빌드 (애플 실리콘 / ARM 서버)
pwsh -File scripts/release.ps1 -Version v0.80.0 -Platform linux/arm64
```
...
```powershell
# 스크립트를 사용하여 릴리즈 업로드
pwsh -File scripts/gh_release.ps1 -Version v0.80.0 -PrevVersion v0.79.8
```

raw `gh release create`로 직접 게시하지 않습니다. 위 스크립트는 clean tree, annotated tag,
`origin/master`와 성공한 CI, 6개 payload의 manifest/hash, tagged canonical
...
```powershell
gh release view v0.80.0 --repo hkjang/vibe-coders
```
...
# 정상: ai-coding-proxy-gateway-v0.80.0.tar.gz: OK
```
...
# 정상: Loaded image: ai-coding-proxy-gateway:v0.80.0
```
