...
func TestAppRequestsSafeProjectionAndStableCursor(t *testing.T) {
	...
	if !strings.Contains(visible, boundedModelsProviderLabel(unsafeProvider)) || !strings.Contains(visible, server.providerRef(unsafeProvider)) {
...
func TestAppRequestsOpenAPIProjectionMatchesRuntimeFields(t *testing.T) {
	...
	if !strings.Contains(string(encoded), "AppRequestsResponse") {
		t.Fatalf("/admin/requests 200 contract = %s", encoded)
	}
}
