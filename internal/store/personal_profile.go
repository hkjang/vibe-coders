...
func (s *SQLStore) BuildPersonalProfile(ctx context.Context, userID string, since time.Time) (PersonalProfile, error) {
	p := PersonalProfile{UserID: userID, Since: since.UTC().Format(time.RFC3339Nano),
		TopTaskTypes: []ProfileCount{}, TopModels: []ProfileCount{}, TopLanguages: []ProfileCount{}}
	...
	err := s.db.QueryRowContext(ctx, s.bind(`
		...
		Scan(&p.Team, &p.Role, &p.Requests, &p.TotalCostKRW, &successes, &p.DistinctModels, &p.DistinctFingerprints)
	if err != nil {
		return p, err
	}
	if p.Requests > 0 {
...
func personalProfileSummary(p PersonalProfile) string {
	...
	return fmt.Sprintf("주로 %s을(를) 수행하며 %s 모델을 선호. 성공률 %.0f%%, 요청당 평균 %.2f KRW (총 %d건).",
		taskStr, topModel, p.SuccessRate*100, p.AvgCostPerRequest, p.Requests)
}
