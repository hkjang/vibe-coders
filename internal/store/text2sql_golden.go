...
func (s *SQLStore) ListText2SQLGoldenQueries(ctx context.Context, onlyEnabled bool) ([]Text2SQLGoldenQuery, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, question, expected_sql, COALESCE(schema_name,''), COALESCE(tags,''), enabled, created_at, updated_at
	...
	for rows.Next() {
		...
		var tags string
		var enabled int
		if err := rows.Scan(&g.ID, &g.Name, &g.Question, &g.ExpectedSQL, &g.SchemaName, &tags, &enabled, &g.CreatedAt, &g.UpdatedAt); err != nil {
			return nil, err
		}
...
func (s *SQLStore) UpsertText2SQLGoldenQuery(ctx context.Context, g Text2SQLGoldenQuery) error {
	...
	_, err := s.db.ExecContext(ctx, s.bind(`INSERT INTO text2sql_golden_queries
		(id, name, question, expected_sql, schema_name, tags, enabled, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET name = excluded.name, question = excluded.question, expected_sql = excluded.expected_sql,
			schema_name = excluded.schema_name, tags = excluded.tags, enabled = excluded.enabled, updated_at = excluded.updated_at`),
		g.ID, g.Name, g.Question, g.ExpectedSQL, g.SchemaName, strings.Join(g.Tags, ","), enabled, g.CreatedAt, now)
	return err
}
