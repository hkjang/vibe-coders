...
$guideLines = @(
    ...
    "   # Reusing a volume or bind mount: restore nonroot (65532) ownership. A fresh volume is left unchanged."
    ...
    ""
    "7. Startup failure diagnosis (port 8080 closed, readonly database)"
    ""
    "   If the container keeps restarting and 8080 never opens, read the logs and check the data directory first."
    ""
    "   ``````bash"
    ...
    "   ``````"
    ""
    "   'data directory /data is not writable' or 'attempt to write a readonly database' means another user (root, a"
    "   bind mount, a Kubernetes PVC) wrote /data. The gateway runs as nonroot (uid 65532) and the image has no shell,"
    "   so repair the ownership once with the same image, verify, and restart."
    ""
    "   ``````bash"
    ...
    "   ``````"
    ""
    "   For a bind mount use -v <host dir>:/data instead of --mount; on Kubernetes set securityContext.fsGroup: 65532"
    "   or run the same command as an initContainer."
)
