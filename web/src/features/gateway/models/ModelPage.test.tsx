...
describe("ModelPage", () => {
  ...
  it("filters through URL state and requests all read-only enrichments with the selected range", async () => {
    ...
    const table = screen.getByRole("table", { name: "공급자별 모델 상태, 품질과 가격" });
  ...
  it("deep-links, filters, and restores focus for duplicate masked Providers by provider_ref", async () => {
    ...
    const table = await screen.findByRole("table", { name: "공급자별 모델 상태, 품질과 가격" });
    ...
    await user.selectOptions(screen.getByLabelText("공급자"), secondRef);
  ...
  it("shows the required shadow status and prioritizing Agent Route in the detail dialog", async () => {
    ...
    expect(within(row).getByText("우선순위 밀림")).toBeVisible();
  ...
  it("does not present previous-range quality as the newly selected range", async () => {
    ...
    expect(await within(loadingDialog).findByText("모델 품질 정보를 불러오는 중입니다.")).toBeVisible();
  ...
  it("disables a model detail retry while the catalogue refresh is running", async () => {
    ...
    const retry = await within(dialog).findByRole("button", { name: "모델 카탈로그 상세 재시도" });
  ...
  it("removes credential searches from deep links and rejects submitted secrets", async () => {
    ...
    const input = screen.getByRole("textbox", { name: "모델 검색" });
    await waitFor(() =>
  ...
  it("scrubs a secret-like legacy model_provider and renders a generic not-found state", async () => {
    ...
    expect(within(dialog).getByText("모델을 찾을 수 없습니다.")).toBeVisible();
  ...
  it("shows catalogue refresh progress inside an open detail dialog", async () => {
    ...
    const dialog = await screen.findByRole("dialog", { name: "gpt-5" });
    expect(await within(dialog).findByText("모델 카탈로그 정보를 갱신하고 있습니다.")).toBeVisible();
    expect(within(dialog).getByText("카탈로그")).toBeVisible();

    resolveModels?.(modelResponse);
    await waitFor(() =>
      expect(within(dialog).queryByText("모델 카탈로그 정보를 갱신하고 있습니다.")).not.toBeInTheDocument(),
    );
  });
  ...
  it("distinguishes an actual empty catalogue from an unavailable one", async () => {
    ...
    expect(
      await screen.findByText("조회 가능한 모델이 없습니다. 공급자 연결과 모델 라우트를 확인하세요."),
    ).toBeVisible();
    expect(within(screen.getByLabelText("모델 요약")).getAllByText("0")).toHaveLength(4);
  });
  ...
  it("hides Legacy links when fallback is unavailable", async () => {
    ...
    expect(screen.queryByRole("link", { name: /기존/ })).not.toBeInTheDocument();
  });
