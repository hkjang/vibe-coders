...
export interface GovernanceColumn<Row extends object> {
  id: string;
  header: string;
  cell: (row: Row) => ReactNode;
}

interface GovernanceTableProps<Row extends object> {
  caption: string;
  columns: ReadonlyArray<GovernanceColumn<Row>>;
  emptyMessage?: string;
  error?: string;
  loading?: boolean;
  onRetry?: () => void;
...
export function GovernanceTable<Row extends object>({
  caption,
  columns,
  emptyMessage,
  error,
  ...
  onRetry,
  rows,
}: GovernanceTableProps<Row>): React.JSX.Element {
