...
/**
 * Horizontal bars drawn with tokens only; the numbers stay readable as text for
 * assistive technology because this is a definition list, not a picture.
 */
export function BarList({ items, label }: { items: readonly BarDatum[]; label: string }): React.JSX.Element {
  const max = items.reduce((peak, item) => Math.max(peak, item.value), 0);
  return (
    <dl className="routing-bars" aria-label={label}>
      {items.map((item) => (
        <div className="routing-bar-row" key={item.label}>
          <dt className="truncate" title={item.label}>
            {item.label}
          </dt>
          <dd>
            <span className="routing-bar-track">
              <span
                className="routing-bar-fill"
...
export function ScopeNotice({ children }: { children: ReactNode }): React.JSX.Element {
  return (
